...
export default function Home({ settings: s }: { settings: S }) {
  ...
  const [stops, setStops] = useState<Array<P | null>>([]);
  const [stopInputs, setStopInputs] = useState<string[]>([]);
  const [stopSuggestions, setStopSuggestions] = useState<P[][]>([]);
  ...
  function searchAddress(value: string, type: "pickup" | "destination") {
    ...
    // The route is no longer valid while the user is editing an endpoint.
    setDistance(0);
    setDuration(0);
    setPrice(null);
  ...
  useEffect(() => {
    const incompleteStop = stopInputs.some(
      (value, index) => value.trim() && !stops[index]
    );

    if (pp && dp && !incompleteStop) {
      calculate();
    }
  }, [pp, dp, stops, stopInputs]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (stopInputs.some((value, index) => value.trim() && !stops[index])) {
      setMsg(
        lang === "nl"
          ? "Selecteer alle ingevulde tussenstops uit de lijst."
          : lang === "fr"
          ? "Sélectionnez tous les arrêts saisis dans la liste."
          : "Please select every entered stop from the list."
      );
      return;
    }
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[1fr_1fr] lg:items-center lg:py-24">
          ...
          >
            ...
            {stopInputs.map((value, index) => (
              <div key={`stop-${index}`} className="relative mb-4">
                <label className="mb-2 block text-sm font-bold">{t.stops} {index + 1}</label>
                <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const next = [...stopInputs];
                      next[index] = value;
                      setStopInputs(next);

                      setStops((current) => {
                        const nextStops = [...current];
                        nextStops[index] = null;
                        return nextStops;
                      });
                      ...
                      setTimeout(async () => {
                        try {
                          ...
                          setStopSuggestions((current) => {
                            const nextSuggestions = [...current];
                            nextSuggestions[index] = d.suggestions || [];
                            return nextSuggestions;
                          });
                        } catch {}
                      }, 300);
                  ...
                  <button type="button" onClick={() => {
                    setStopInputs((current) => current.filter((_, i) => i !== index));
                    setStopSuggestions((current) => current.filter((_, i) => i !== index));
                    setStops((current) => current.filter((_, i) => i !== index));
                    setDistance(0);
                    setDuration(0);
                    setPrice(null);
                  }} className="shrink-0 rounded-xl px-2 py-1 text-xs font-bold text-red-600">{t.removeStop}</button>
                </div>
                {(stopSuggestions[index] || []).length > 0 && (
                  <div className="absolute right-0 left-0 z-20 mt-2 overflow-hidden rounded-2xl border bg-white shadow-2xl">
                    {(stopSuggestions[index] || []).map((place) => (
                      <button type="button" key={place.placeId} onClick={() => {
                        const next = [...stops];
                        next[index] = place;
                        setStops(next);
                        const inputs = [...stopInputs];
                        inputs[index] = place.text;
                        ...
                        <MapPin size={17} className="mt-1 shrink-0 text-gray-400" />
                        <span><b className="block">{place.text}</b><small className="text-gray-500">{place.secondary}</small></span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <button
              type="button"
              onClick={() => {
                if (stopInputs.length >= 5) return;
                setStopInputs((current) => [...current, ""]);
                setStopSuggestions((current) => [...current, []]);
                setStops((current) => [...current, null]);
                setDistance(0);
                setDuration(0);
                setPrice(null);
              }}
              className="mb-4 w-full rounded-2xl border border-dashed border-gray-300 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50"
            >
              + {t.addStop}
            </button>
            ...
            {pp && (stops.some(Boolean) || dp) && (
              <div className="mb-4 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm">
                <div className="mb-2 font-bold text-gray-700">
                  {lang === "nl" ? "Routevolgorde" : lang === "fr" ? "Ordre du trajet" : "Route order"}
                </div>
                <div className="flex flex-wrap items-center gap-2 text-gray-600">
                  <span className="font-semibold">{pickup || t.from}</span>
                  {stops.map((stop, index) =>
                    stop ? (
                      <span key={`${stop.placeId}-${index}`} className="flex items-center gap-2">
                        <span>→</span>
                        <span className="font-semibold">{stop.text}</span>
                      </span>
                    ) : null
                  )}
                  {dp && (
                    <span className="flex items-center gap-2">
                      <span>→</span>
                      <span className="font-semibold">{destination || t.to}</span>
                    </span>
                  )}
                </div>
              </div>
